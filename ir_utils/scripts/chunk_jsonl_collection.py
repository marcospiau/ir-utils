...
parser = ArgumentParser(
    description="Chunk jsonl collection", formatter_class=ArgumentDefaultsHelpFormatter
)
...
sharding_mutually_exclusive_group.add_argument(
    ...
    required=False,
    default=None,
    help="The number of output shards to write. Defaults to None (single shard).",
)
sharding_mutually_exclusive_group.add_argument(
    "--max_rows_per_shard",
    type=int,
    required=False,
    default=None,
    help="The maximum number of rows per shard. Defaults to None (single shard).",
)
...
def main():
    ...
    logging.info("Loaded %d documents from %s", len(df_chunks), args.input_collection)
    ...
    df_chunks = df_chunks.select(
        pl.col("segment_id").alias("doc_id"), pl.col("segment_text").alias("doc_text")
    )
