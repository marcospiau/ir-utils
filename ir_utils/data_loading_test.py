...
from ir_utils.data_loading import (
    check_keys_match,
    load_batched_filtered_docs_from_anserini_index,
    load_filtered_docs_from_anserini_index,
    load_filtered_jsonl_docs,
    load_top_k_query_doc_pairs,
    load_tsv_queries,
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            {"query1": "What is the capital of France?"},
            pl.DataFrame(
                {"qid": ["query1"], "query": ["What is the capital of France?"]}
            ),
        ),
        ("", {}, pl.DataFrame(schema=QueryPTModel.dtypes)),
    ],
)
def test_load_tsv_queries(tsv_content, expected_output_dict, expected_output_df):
    with tempfile.NamedTemporaryFile(mode="w+", delete=True) as tmpfile:
        ...
        pl.testing.assert_frame_equal(
            output_dataframe, expected_output_df, check_row_order=False
        )
...
def test_load_top_k_query_doc_pairs(run_content, top_k, expected_output):
    ...
    with patch("builtins.open", m):
        assert load_top_k_query_doc_pairs("fake_path", top_k) == expected_output
...
@pytest.mark.parametrize(
    ...
    [
        ({"key1": "value1", "key2": "value2"}, {"key1", "key2"}, None),
        (
            {"key1": "value1", "key2": "value2"},
            {"key1", "key3"},
            "Data keys and expected keys mismatch",
        ),
    ],
)
...
def test_load_filtered_jsonl_docs_no_check():
    ids = {"doc1", "doc2"}
    mock_data = (
        '{"id": "doc1", "contents": "Document text 1"}\n'
    ...
    with (
        ...
        patch("glob.glob", return_value=["mock_file.jsonl"]),
    ):
...
def test_load_filtered_docs_from_anserini_index_with_valid_data():
    ...
    with (
        mock.patch("ir_utils.data_loading.IndexReader", return_value=mock_index),
        mock.patch("ir_utils.data_loading.check_keys_match"),
    ):
        result = load_filtered_docs_from_anserini_index(set(doc_ids), "dummy_index_dir")
...
def test_load_batched_filtered_docs_from_anserini_index_with_valid_data():
    ...
    with (
        mock.patch("ir_utils.data_loading.LuceneSearcher", return_value=mock_searcher),
        mock.patch("ir_utils.data_loading.check_keys_match"),
    ):
